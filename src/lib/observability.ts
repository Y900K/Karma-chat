...
export async function recordOperationalError(input:{service:string;code:string;route:string;requestId:string;severity?:"info"|"warning"|"error"|"critical";metadata?:Json}){
  ...
  const {error}=await admin.from("operational_errors").insert({service:input.service,error_code:input.code,route:input.route,severity:input.severity||"error",request_id:input.requestId,metadata:input.metadata||{}}).abortSignal(AbortSignal.timeout(3000));
  if(error)console.error("Operational error persistence failed",{code:error.code});
}
